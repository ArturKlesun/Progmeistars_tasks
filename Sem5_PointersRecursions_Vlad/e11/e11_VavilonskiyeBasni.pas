...
//Эта процедура перемещает нужное количество столбиков вправо, но пока кол-во столбиков ограничено единицей
var p,pp:PElement;
Begin
        if n>1 then
...
Begin
        Writeln;
        Writeln;
        Writeln('Эта программа перемещает пирамидку с А на В.');
        Writeln('Введите, пожалуйста, высоту пирамидки: ');
        Writeln('Core2Duo:');
        Writeln('26 - 2 секунды;');
        Writeln('27 - 4 секунды;');
        Writeln('28 - 8 секунд;');
        Readln(etotX);
        gd:=detect;
        initgraph(gd,gm,'');
